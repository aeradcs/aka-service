...
Список всех urls доступен по адресу http://<your_ip>:<your_port>/docs/ после настройки сервера (см. раздел ниже).
...
# Aka Service | installation on Linux

1. 
        git clone https://github.com/aeradcs/aka-service.git
...
8. В файле settings.py в строке ALLOWED_HOSTS нужно указать ip-адрес, на котором будет работать сервис. 
...
11. Настроить автоперезапуск сервера в случае падения (**если вы настроили сервер по шагу 10, то сначала его необходимо отключить** -- kill <pid_num>):

- В каталоге /aka-service/service лежит 2 файла – cron.sh и run.sh.

           1) В cron.sh нужно поменять путь "/home/nalepova" в строке 

                cd /home/nalepova/aka-service/service 

           на тот, по которому у вас лежит каталог aka-service/service.

           2.1) В run.sh нужно поменять команду
           ...
           **Если структура проекта при клонировании не была изменена, то эту строку менять не нужно.**
           ...
           2.2) APP_NAME="python3 manage.py runserver 0.0.0.0:7710"

                7710 – нужно поменять на ваш порт.

- Добавить права на запуск run.sh:
...
- Проверка работы сервера:
   ...
   Если все настроено правильно, то команда выдаст:
        ...
        python3 manage.py runserver 0.0.0.0:<your_port> is running with PID: <pid_num>
